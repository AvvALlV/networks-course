...
5. Когда HTML-файл, который вы извлекаете, последний раз модифицировался на сервере?
   - Mon, 26 Feb 2024 06:59:02 GMT
6. Сколько байтов контента возвращается вашему браузеру?
   - 128

![alt text](screenshots/1_get_request.png)
![alt text](screenshots/1_get_response.png)
...
![alt text](screenshots/2.1_get_request.png)
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - да
   - можно посмотреть текстовые данные

![alt text](screenshots/2.1_get_response.png)
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - последняя модификация документа: Mon, 26 Feb 2024 06:59:02 GMT

![alt text](screenshots/2.2_get_request.png)
...
![alt text](screenshots/2.2_get_response.png)
...
![alt text](screenshots/3_response.png)
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - параллельно, можно посмотреть на время выполнения get-запросов. Сначала был отправлен get-запрос на первую картинку, потом на вторую. После чего пришло два соответствующих ответа. При последовательной загрузке второй get-запрос был бы отправлен, после получения первого изображения.

![alt text](screenshots/4_info.png)
...
![alt text](screenshots/5_info.png)