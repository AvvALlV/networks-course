...
3. Обновить соответствующий продукт

![alt text](images/put_product.png)

4. Получить список всех продуктов

![alt text](images/get_products.png)

5. Удалить соответствующий продукт
...
#### Решение
В связи с ограничением в 100 Кбит/c, то максимальное количество одновременных клиентов равно: $\dfrac{2000}{100} = 20$. Тогда вероятность для 12 одновременных клиентов равна $C^{12}_{60} * 0.2^{12} * 0.8 ^ {60 -12}$, следовательно для 12 и более: $\sum_{i = 12}^{20} C^{i}_{60} * 0.2^{i} * 0.8 ^ {60 -i} = 0.55$
